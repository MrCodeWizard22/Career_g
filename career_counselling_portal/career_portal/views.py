...
def sendOTP(request):
    if request.method == 'POST':
        try:
            ...
            send_mail(subject, message, from_email='noreplytestprofile@gmail.com', recipient_list=[email])
...
def sendVerificationEmail(request):
    if request.method == 'POST':
        try:
            ...
            message= f"Dear {name}, We Hope this message finds you well. Your registration on CareerGuidance is currently being processed. Our team is diligently verifying the provided information, and we anticipate completing the verification soon. Once the verification process is complete, you will receive a confirmation email along with further details about accessing your account. Thank you for choosing CareerGuidance."
            send_mail(subject, message, from_email='noreplytestprofile@gmail.com', recipient_list=[email])
...
def deleteCounsellor(request, userEmail,rejectionReason):
    if request.method == 'DELETE':
        try:
            ...
            message = f"Dear {counsellor.name},\n\nWe hope this message finds you well. Your registration on CareerGuidance is being rejected due to the following reason:\n{rejectionReason}\n"
            send_mail(subject, message, from_email='noreplytestprofile@gmail.com', recipient_list=[userEmail])
            counsellor.delete()   
    ...
    if request.method == 'GET':
        try:
            ...
            if counsellor_counsellor:
                ...
                message = f"Dear {counsellor.name},\n\nWe hope this message finds you well. Your registration on CareerGuidance has been accepted. Kindly accept a warm welcome from our administration:\n`{greetingMessage}`\n"
                send_mail(subject, message, from_email='noreplytestprofile@gmail.com', recipient_list=[userEmail])
